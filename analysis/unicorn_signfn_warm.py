...
def main():
    ...
    # ------------------------------------------------------------------
    # Pre-load captured native heap ranges from /tmp/op60_memdump.json
    # The hypothesis: many heap-resident structures used by wrapper.node
    # are populated by the sign() prologue. By pre-seeding them with the
    # bytes we captured at op60 entry, the early sign_fn setup code is
    # more likely to find the data structures in a 'valid' shape and
    # avoid the early corrupt-indirect-call failures.
    MEMDUMP_JSON = '/tmp/op60_memdump.json'
    PROT_MAP = {
        'r--': UC_PROT_READ,
        'r-x': UC_PROT_READ | UC_PROT_EXEC,
        'rw-': UC_PROT_READ | UC_PROT_WRITE,
        'rwx': UC_PROT_READ | UC_PROT_WRITE | UC_PROT_EXEC,
        '---': 0,
    }
    try:
        memdump = json.load(open(MEMDUMP_JSON))
        cap_wb = memdump['wrapper_base']
        cap_we = cap_wb + 0x7dc0818
        print(f"[+] op60_memdump: capture wrapper_base=0x{cap_wb:x}, "
              f"capture end=0x{cap_we:x}, {len(memdump['ranges'])} ranges")
        ...
        warm_skip_arena = 0
        warm_fail = 0
        for r in memdump['ranges']:
            a = r['addr']; sz = r['size']
            if sz <= 0:
                continue
            ae = a + sz
            # Skip ranges that overlap the captured wrapper.node region
            # (those bytes ARE wrapper.node; we already have it mapped at
            #  WBASE=0..WRAPPER_END from disk).
            if not (ae <= cap_wb or a >= cap_we):
                warm_skip_wrapper += 1
                continue
            # Skip ranges that overlap our arena (we already own that)
            if not (ae <= SHARED_ARENA_VA or a >= SHARED_ARENA_VA + SHARED_ARENA_SIZE):
                warm_skip_arena += 1
                continue
            # Skip if it would overlap our WBASE=0 wrapper.node mapping
            if not (ae <= WBASE or a >= WRAPPER_END):
                warm_skip_wrapper += 1
                continue
            # Load file bytes
            try:
                ...
                warm_fail += 1
                continue
            page_start = align_down(a)
            page_end = align_up(ae)
            perms = PROT_MAP.get(r.get('prot', 'rw-'),
                                  UC_PROT_READ | UC_PROT_WRITE)
            # Force RW at minimum so later code can mutate
            ...
            try:
                ...
                # Already mapped (page collision with another captured
                # range) — try to just write.
                try:
                    mu.mem_write(a, data)
                    warm_overwrote += 1
                except UcError:
                    warm_fail += 1
        print(f"[+] warm-load: mapped={warm_mapped} overwrote={warm_overwrote} "
              f"skip_wrap={warm_skip_wrapper} skip_arena={warm_skip_arena} "
              f"fail={warm_fail}")
    except (FileNotFoundError, OSError) as e:
        print(f"[!] could not load memdump: {e}")
    ...
    # Also probe the well-known cipher target address 0x24922b0
    # (within wrapper.node range — should be writable RW page).
    CIPHER_TARGET = 0x24922b0
    try:
        cipher_bytes = bytes(mu.mem_read(CIPHER_TARGET, 0x20))
        print(f"  [0x{CIPHER_TARGET:x}..+0x20]: {cipher_bytes.hex()}")
        if any(b != 0 for b in cipher_bytes):
            print(f"  (non-zero — possible cipher write happened)")
    except UcError as e:
        print(f"  could not read 0x{CIPHER_TARGET:x}: {e}")
