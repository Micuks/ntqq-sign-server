...
last_jump_log = []
block_set = set()       # 64KB-aligned blocks (addr & ~0xFFFF)
page_set = set()        # 4KB-aligned pages (addr & ~0xFFF)
def hook_code(uc, address, size, user_data):
    exec_count[0] += 1
    # Coverage tracking — record every executed insn's 64KB block and 4KB page
    block_set.add(address & ~0xFFFF)
    page_set.add(address & ~0xFFF)
...
# ============================================================
# Coverage report: which wrapper.node regions did we actually hit?
# ============================================================
print("\n" + "=" * 60)
print("CODE COVERAGE REPORT")
print("=" * 60)
print(f"Total executed instructions:       {exec_count[0]}")
print(f"Total unique 64KB blocks visited:  {len(block_set)}")
print(f"Total unique 4KB pages visited:    {len(page_set)}")

# Important reference points (file offsets / VA inside wrapper.node)
REFERENCE_POINTS = [
    ("sign_fn entry",                 0x56D81D1),
    ("block1 cipher sub_56C46C0",     0x56C46C0),
    ("VM dispatcher sub_56B4244",     0x56B4244),
    ("op 0x60 outer sub_5CCD94A",     0x5CCD94A),
    ("op 0x60 helper sub_5CE6006",    0x5CE6006),
]

print(f"\nVisited 64KB blocks (sorted), wrapper offsets shown:")
for b in sorted(block_set):
    if WRAPPER_START <= b < WRAPPER_END:
        off = b - WBASE
        print(f"  0x{b:016x}  (wrapper offset 0x{off:08x})")
    else:
        print(f"  0x{b:016x}  (NON-wrapper region)")

print(f"\nReference-point coverage check:")
for name, off in REFERENCE_POINTS:
    va = WBASE + off
    block_va = va & ~0xFFFF
    hit = block_va in block_set
    page_va = va & ~0xFFF
    page_hit = page_va in page_set
    status = "HIT " if hit else "MISS"
    print(f"  [{status}] {name:<35s}  off=0x{off:08x}  va=0x{va:x}  block=0x{block_va:x}  page_hit={page_hit}")

# Also: did we visit any block in the 0x56c4xxxx (block1 cipher) or 0x56b4xxxx (VM dispatcher) area?
cipher_blocks = sorted([b for b in block_set if (b - WBASE) >> 16 == 0x56c4 >> 0 or
                                                  ((b - WBASE) & 0xFFFF0000) == 0x05c40000])
vm_blocks     = sorted([b for b in block_set if ((b - WBASE) & 0xFFFF0000) == 0x05b40000])
op60_blocks   = sorted([b for b in block_set if ((b - WBASE) & 0xFFFF0000) in (0x05cc0000, 0x05ce0000)])
sign_blocks   = sorted([b for b in block_set if ((b - WBASE) & 0xFFFF0000) == 0x056d0000])

def fmt(blks):
    return ", ".join(f"0x{(b - WBASE):08x}" for b in blks) if blks else "(none)"

print(f"\nBlocks in cipher region (offset 0x56c4xxxx range): {fmt(cipher_blocks)}")
print(f"Blocks in VM-dispatcher region (offset 0x56b4xxxx range): {fmt(vm_blocks)}")
print(f"Blocks in op-0x60 region (offset 0x5ccxxxxx/0x5cexxxxx): {fmt(op60_blocks)}")
print(f"Blocks in sign-fn region (offset 0x56d0xxxx): {fmt(sign_blocks)}")
