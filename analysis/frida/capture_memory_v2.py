"""Capture memory reads at the VM dispatcher.

For op 0x29 and op 0x32, the index register often points into a buffer.
We capture the address being read and the value loaded, plus context.
"""
                ...
                while ((ins = iter.next()) !== null) {
                    if (ins.address.equals(WRAPPER_BASE.add(DISPATCHER_OFF))) {
                        iter.putCallout(function(ctx) {
                            if (memReads.length >= 17000) return;
                            const rax = ctx.rax;
                            const r14 = ctx.r14;
                            let ib = null;
                            try {
                            ...
                            // Try to capture key registers AND certain memory at common buffer pointers
                            let regs = null;
                            try {
                                const ra = r14.add(0x10).readPointer();
                                const arr = new Uint32Array(NREGS);
                                for (let i = 0; i < NREGS; i++) {
                                    try { arr[i] = ra.add(i*8).readU32(); } catch(e) { arr[i] = 0; }
                                }
                                regs = Array.from(arr);
                            } catch(e) {}
                            // For op 0x29, ib[2] is base reg. For op 0x32, similar.
                            // Try reading 16 bytes at ptr(reg[ib[1]]), ptr(reg[ib[2]]), ptr(reg[ib[3]]) if they look like pointers
                            let memSnap = {};
                            if (ib && regs) {
                                for (const k of [1, 2, 3]) {
                                    ...
                                    if (r < NREGS) {
                                        const v = regs[r];
                                        if (v > 0x100000 && v < 0xffffffff - 1000) {
                                            try {
                                                const data = ptr(v).readByteArray(8);
                                                memSnap['r'+r] = Array.from(new Uint8Array(data));
                                            } catch(e) {}
                                        }
                                    }
                                }
                                // Also try as 64-bit ptr (high reg + low reg)
                            }
                            memReads.push([stepIdx++, pc, ib, regs, memSnap]);
                        });
                    }
                    iter.keep();
                }
...
def main():
    ...
    def on_msg(msg, data):
        if msg['type'] == 'send':
            ...
            if pl.get('type') == 'count':
                print(f"[s] capturing {pl['n']} steps")
            elif pl.get('type') == 'chunk':
                start = pl['start']
                for i, s in enumerate(pl['data']):
                    samples[start + i] = s
                n_recv[0] += len(pl['data'])
    ...
    out_path = f'/tmp/mem_trace_{src_hex}.json'
