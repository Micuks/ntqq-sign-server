"""Capture many op 0x1a (r13, target_after) mappings by running sign() with many src bytes.

For each sign() call, we get ~64 op 0x1a invocations. With 256 src bytes, we get ~16K mappings.
"""
import frida, subprocess, os, time, json, sys

NUM_TRACES = 256  # all single-byte src values
...
let inside = false;
let tid = null;
let pendingOp1a = null;  // saved r13 from start
const allMappings = [];
let currentMappings = [];
...
Interceptor.attach(SIGN_FN, {
                    ...
                    if (ins.address.equals(WRAPPER_BASE.add(DISPATCHER_OFF))) {
                        iter.putCallout(function(ctx) {
                            try {
                                ...
                                // If we had a pending op 0x1a, capture target_after now
                                if (pendingOp1a !== null) {
                                    const r46_after = ra.add(46*8).readU32();
                                    currentMappings.push([pendingOp1a, r46_after]);
                                    pendingOp1a = null;
                                }

                                // If this is op 0x1a, save r13
    ...
    onLeave: function() {
        ...
        inside = false;
        // Save last pending if any
        allMappings.push(currentMappings);
    }
});

recv('dump', function() {
    let total = 0;
    for (const m of allMappings) total += m.length;
    send({type: 'count', total: total, runs: allMappings.length});
    const CHUNK = 200;
    let flat = [];
    for (const m of allMappings) flat = flat.concat(m);
    for (let i = 0; i < flat.length; i += CHUNK) {
        send({type: 'chunk', data: flat.slice(i, i+CHUNK)});
    }
    send({type: 'done'});
});
...
for line in sys.stdin:
    ...
    if line.startswith('SIGN_'):
        ...
        r = sign_once(sb)
        print(f'SIGN_RESULT_{sb:02x}', flush=True)
...
def main():
    ...
    def on_msg(msg, data):
        if msg['type'] == 'send':
            ...
            if pl.get('type') == 'count':
                print(f"[s] captured {pl['total']} mappings across {pl['runs']} runs")
    ...
    for sb in range(NUM_TRACES):
        ...
        while True:
            ...
            if line.startswith(f'SIGN_RESULT_{sb:02x}'):
                break
        if (sb+1) % 32 == 0:
            print(f'  [progress] {sb+1}/{NUM_TRACES} signed')
    ...
    print(f'\nCaptured {len(samples)} mappings')
    with open('/tmp/op1a_mass.json','w') as f:
        json.dump(samples, f)
    print('Saved /tmp/op1a_mass.json')
