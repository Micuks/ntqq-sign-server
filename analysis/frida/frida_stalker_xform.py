#!/usr/bin/env python3
"""Stalker transform: instrument every memory-write instruction during sign.
Log writes to the 257-byte buffer holding the XOR stream."""
...
let targetBufStart = null;
let targetBufEnd = null;
let writeLog = [];
let followingTid = null;
...
Interceptor.attach(SIGN_FN, {
    ...
    onLeave: function() {
        if (followingTid) {
            try { Stalker.unfollow(followingTid); Stalker.flush(); } catch(e) {}
            followingTid = null;
        }
        insideSign = false;
        send({type: 'done', writeLog: writeLog.slice(0, 100)});
    }
});

// When size=257 malloc happens, record buffer range and start Stalker
...
Interceptor.attach(mallocPlt, {
    ...
    onLeave: function(ret) {
        if (!insideSign || this.size !== 257 || targetBufStart !== null) return;
        targetBufStart = ret;
        targetBufEnd = ret.add(64);  // watch first 64 bytes
        followingTid = Process.getCurrentThreadId();
        send({type: 'log', msg: 'target buf = ' + ret + ' to ' + targetBufEnd});

        try {
            Stalker.follow(followingTid, {
                transform: function (iterator) {
                    let instr;
                    while ((instr = iterator.next()) !== null) {
                        // Check if this instruction is a memory write
                        const mnem = instr.mnemonic;
                        // Common write instructions: mov, movzx, movsx, lea(with storage), stos*, etc.
                        if (mnem === 'mov' || mnem === 'movdqu' || mnem === 'movaps' || mnem === 'movups') {
                            const ops = instr.operands;
                            // Check if first operand is memory
                            if (ops.length === 2 && ops[0].type === 'mem') {
                                // Insert callout before instruction
                                iterator.putCallout(function(context) {
                                    try {
                                        // Compute effective address; use Capstone-exposed access for ops
                                        // Instead, inspect the 64 bytes at target buffer to see if this instruction modified it
                                        // (we can't cheaply decode effective addr here)
                                        const buf = new Uint8Array(targetBufStart.readByteArray(20));
                                        // Compare with TARGET
                                        let matches = 0;
                                        for (let j = 0; j < 20; j++) if (buf[j] === TARGET[j]) matches++;
                                        if (matches >= 10 && writeLog.filter(w => w.pc === context.pc.toString()).length < 5) {
                                            writeLog.push({
                                                pc: context.pc.toString(),
                                                pcRel: rel(context.pc),
                                                matches: matches,
                                                buf: Array.from(buf)
                                            });
                                        }
        ...
        } catch(e) { send({type: 'log', msg: 'follow err: ' + e.message}); }
    }
});
...
def main():
    ...
    print('[main] triggering sign (this may take longer due to Stalker overhead)...')
    p.stdin.write('SIGN\n'); p.stdin.flush()
    while True:
        line = p.stdout.readline().strip()
        print(f'[helper] {line}')
    ...
    log = result.get('log', [])
    print(f'\n[main] {len(log)} write events captured:')
    for ev in log[:20]:
        print(f"  pc={ev['pcRel']} matches={ev['matches']}/20 buf={bytes(ev['buf']).hex()}")
