#!/usr/bin/env python3
"""Scan all process-readable memory for the VM bytecode pattern.

Pattern: first 8 bytes of bytecode (from OLD trace) = 02 00 15 00 75 01 12 00
This is unique enough to identify the bytecode buffer.
"""
...
Interceptor.attach(HELPER, {
    onEnter: function(args) {
        ...
        send({type: 'progress', msg: `Scanning ${ranges.length} readable memory ranges`});
        const matches = [];
        let scanned_bytes = 0;
        for (const r of ranges) {
            // Skip wrapper.node text (known)
            if (r.base.compare(WRAPPER_BASE) >= 0 && r.base.compare(WRAPPER_BASE.add(0x10000000)) < 0) continue;
            // Skip extremely large ranges to keep this fast
            ...
            try {
                ...
                for (const m of found) {
                    matches.push({
                        ...
                        in_wrapper: false,
                        size: r.size,
                        // Read 32 bytes context
                        context: Array.from(new Uint8Array(m.address.readByteArray(32)))
                    });
                }
                scanned_bytes += r.size;
            } catch(e) {}
        }
        send({type: 'scan', matches: matches, scanned_bytes: scanned_bytes});
    }
});
...
def main():
    ...
    def on_msg(msg, data):
        if msg['type'] == 'send':
            pl = msg['payload']
            if pl.get('type') == 'progress':
                print(f"[script] {pl['msg']}")
            elif pl.get('type') == 'scan':
                matches_received.extend(pl['matches'])
                print(f"[script] Scanned {pl['scanned_bytes']/1e6:.1f} MB; found {len(pl['matches'])} matches")
    ...
    print(f'\n=== Total matches: {len(matches_received)} ===')
    for m in matches_received[:20]:
        ctx_hex = ' '.join(f'{b:02x}' for b in m['context'])
        print(f"  addr={m['addr']} size={m['size']} bytes_at_match={ctx_hex}")
