"""Capture VM traces for MANY src bytes in ONE PROCESS — eliminates per-process pointer noise."""
import frida, subprocess, os, time, json, sys

NUM_TRACES = 16  # capture for src bytes 0..15
SCRIPT = r"""
'use strict';
...
def main():
    ...
    def on_msg(msg, data):
        if msg['type'] == 'send':
            ...
            if pl.get('type') == 'count':
                print(f"[script] {pl['n_traces']} traces, {pl['n_per']} samples each")
    ...
    for sb in range(NUM_TRACES):
        ...
        out_path = f'/tmp/multi_trace_{sb:02x}.json'
